     ...
     for user in root.getchildren():
         ...
         for userabout in user.getchildren():
             if(fl):
                 ...
                 passengers_values.append((insert_user['id'], insert_user['firstname'], insert_user['lastname']))
                 if len(passengers_values) > 100:
                     insert = sql.SQL('INSERT INTO Passanger (id, first_name, last_name) VALUES {}').format(
                         sql.SQL(',').join(map(sql.Literal, passengers_values))
                     )
                 ...
                 for card in userabout.getchildren():
                     ...
                     # вставка документов
                     document_values.append((insert_documents['id'], insert_documents['numbercart']))
                     if len(document_values) > 100:
                         insert = sql.SQL('INSERT INTO Passangers_document (id, Number_card) VALUES {}').format(
                             sql.SQL(',').join(map(sql.Literal, document_values))
                         )
                     ...
                     for cardabout in card:
                         if(fl):
                             ...
                             for activity in cardabout.getchildren():
                                 ...
                                 # airport_values.append((insert_airoport_to['code'],''))
                                 # # вставка аэропорта улета
                                 # airport_values.append((insert_airoport_from['code'],''))
                                 # if len(airport_values) > 100:
                                 #     insert = sql.SQL('INSERT INTO Airport (code,city) VALUES {}').format(
                                 #         sql.SQL(',').join(map(sql.Literal, airport_values))
                                 #     )
                                 #     try:
                                 #         cursor.execute(insert)
                                 #     except:
                                 ...
                                 flight_values.append((insert_fligth['bonusprogramm'], insert_fligth['Code'],insert_fligth['date'],insert_fligth['airport_to'],insert_fligth['airport_from']))
                                 if len(flight_values) > 100:
                                     insert = sql.SQL('INSERT INTO Fligth (bonus_program, flight_code, date, airport_to,airport_from) VALUES {}').format(
                                         sql.SQL(',').join(map(sql.Literal, flight_values))
                                     )
                                     cursor.execute(insert)
                                     flight_values = []
                                 ...
                                 on_flight_values.append((insert_passger_on_flight['flight_code'], insert_passger_on_flight['passage_code']))
                                 if len(on_flight_values) > 100:
                                     insert = sql.SQL('INSERT INTO Passanger_on_flight (flight_code, pasanger_code) VALUES {}').format(
                                         sql.SQL(',').join(map(sql.Literal, on_flight_values))
                                     )
                                     cursor.execute(insert)
                                     on_flight_values = []


# основной цикл завершился, но в массивах еще могут быть данные
...
if __name__ == "__main__":
    parseXML("/home/yznik/Downloads/wtf/PointzAggregator-AirlinesData.xml")
    cursor.close()
    connection.close()